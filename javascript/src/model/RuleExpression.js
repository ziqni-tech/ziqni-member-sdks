...
class RuleExpression {
    ...
    /**
     * Validates the JSON data with respect to <code>RuleExpression</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RuleExpression</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of RuleExpression.RequiredProperties) {
        ...
        if (data['rules']) { // data not null
            ...
            for (const item of data['rules']) {
                RuleExpression.validateJSON(item);
            };
        }
        if (data['then']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['then'])) {
                throw new Error("Expected the field `then` to be an array in the JSON data but got " + data['then']);
            }
            // validate the optional field `then` (array)
            for (const item of data['then']) {
                RuleRoutine.validateJSON(item);
            };
        }
...
RuleExpression.RequiredProperties = ["type"];
